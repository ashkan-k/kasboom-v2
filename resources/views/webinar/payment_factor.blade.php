    ...
    <div class="container">
      <div class="factor-inner">
        <div class="factor-title">
          <h2> فاکتور ثبت نام وبینار آموزشی</h2>
        </div>
        ...
        <div class="factor-body">
          ...
          <ul class="list">
            ...
            <li id="tr_discount">
              ...
              <span class="desc">{{$payment->discount_percent}} درصد برابر با {{number_format($payment->discount_price)}}<small class="toman">تومان</small></span>
            </li>
            <li>
            ...
            <li>
              ...
              <span class="desc">{{Auth::user()->wallet>0 ? number_format(Auth::user()->wallet)." تومان " : 0}} </span>
            </li>
            <li>
              <span class="name">وضعیت پرداخت</span>
              @if($payment->status ==0)
          ...
          <div class="factor-footer">
            @if($payment->status ==1 && isset($webinar))
            <a href="skill/webinar/{{$webinar->id}}/{{str_replace(' ','_',$webinar->title)}}" class="btn btn-gradient icon-right"><i class="mdi mdi-credit-card-outline"></i> مشاهده وبینار</a>
            @endif
            <button type="button" class="btn btn-info" onClick="javascript:window.print();"><i class="fa fa-file-pdf-o"></i>دریافت فایل pdf</button>
          </div>
        </div>
      </div>
    </div>
