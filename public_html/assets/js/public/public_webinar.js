...
$("#btn_filter_ajax_webinar").click(function () {
  ...
  formDataa.append('all', all);
  formDataa.append('free', free);
  formDataa.append('nofree', nofree);
  // formDataa.append('type_online', type_online);
  // formDataa.append('type_offline', type_offline);

  $.ajax({
    url: "skill/filter_webinar_ajax",
    type: 'POST',
    cache: false,
    data: formDataa,
    contentType: false,
    ...
    beforeSend: function () {
      ShowWaiting("محدودسازی وبینارهای  آموزشی");
    },
...
$("#btn_search_ajax_webinar").click(function () {
  var course_name = $("#webinar_name").val();
  ...
  if (course_name.length < 2)
    ...
    $.ajax({
      ...
      beforeSend: function () {
        ShowWaiting("جستجوی وبینار آموزشی");
      },
      success: function (result) {
        // console.log(result);
        if (!result.error) {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (!result.error) {
          ...
          var new_wallet_online = new_price - user_wallet;
